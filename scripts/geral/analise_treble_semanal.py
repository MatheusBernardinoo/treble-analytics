...
"""
=====================================================================
 ANÁLISE DE MÉTRICAS TREBLE — relatório semanal de atendimento
 Nexforce | RevOps
=====================================================================
...
Saídas:
  ...
  - pasta graficos/treble/ com os PNGs individuais
...
ARQUIVO = None     # Windows: use r"..." ou barras /
...
SEMANA_INICIO = None   # AAAA-MM-DD ou None
...
ARQUIVO_SAIDA = "relatorio_treble_semanal.xlsx"
PASTA_GRAFICOS = "graficos/treble"

# --- Exclusões (PREENCHER conforme o seu fluxo) ---
...
def consolidar_atendimento(dados, excluir_teste=EXCLUIR_TELEFONES_TESTE):
    ...
    # vendedor canônico (1º atribuído) e ORIGEM da última transferência.
    # Dicionário de dados (conferido em 06/07/2026): last_transfer_from é o
    # e-mail de quem ORIGINOU a transferência (enviou para outro vendedor).
    # O nome antigo "recebeu_transfer_de_sistema" invertia essa semântica.
...
# =====================================================================
# B. RESPOSTA NA JANELA 24h — POR DIA E POR SEMANA
# =====================================================================
...
def resposta_24h_semanal(at):
    ...
    col_assign = achar_coluna(at, "assigned_at")
    base = at.dropna(subset=[col_assign]).copy()
    # W-SAT = semana que termina no sábado → começa no domingo
...
def _linhas_ficha(sub, others, n_out_ativos, ti_v, ti_others_avg):
    ...
    return [
        ...
        ("Recebidas via transferência", transf_v, media(transf_o)),
        ("Transferidas (out, originou)", ti_v, round(ti_others_avg, 1)),
...
def gerar_graficos(g, v, at, ib, blocos):
    ...
    if len(at):
        ...
        # 24h DIÁRIO seg–sex — padrão do relatório SEMANAL desde 06/07/2026.
        # O fluxo não recebe novos leads no fim de semana; sábado/domingo só
        # entram no eixo se tiverem chegadas (> 0), para nunca esconder dado.
        try:
            td = blocos.get("B_24h_diario")
            ini_sem = globals().get("_SEMANA_INI")
            if td is not None and len(td) and ini_sem is not None:
                td = td.copy(); td["dia"] = pd.to_datetime(td["dia"])
                uteis = pd.date_range(ini_sem, periods=7, freq="D")
                uteis = [d for d in uteis if d.weekday() < 5]  # seg(0)–sex(4)
                base_d = (td.set_index("dia").reindex(uteis).fillna(0)
                            .rename_axis("dia").reset_index())
                extras = td[(~td["dia"].isin(uteis)) & (td["chegaram"] > 0)]
                if len(extras):
                    base_d = (pd.concat([base_d, extras], ignore_index=True)
                                .sort_values("dia").reset_index(drop=True))
                cheg_d = base_d["chegaram"].astype(int).values
                at24_d = base_d["respondidos_24h"].astype(int).values
                labels_d = [d.strftime("%d/%m") for d in base_d["dia"]]
                x = np.arange(len(base_d))
                # absoluto: barra verde (atendidas) DENTRO da ciano (chegaram)
                fig, ax = plt.subplots(figsize=(9.5, 4.6))
                b1 = ax.bar(x, cheg_d, 0.62, color=COR_CIANO, label="chegaram ao vendedor")
                b2 = ax.bar(x, at24_d, 0.34, color=COR_OK, label="conversas atendidas")
                _ax(ax, "Chegada ao vendedor x conversas atendidas (diário, seg–sex)")
                ax.set_ylabel("conversas")
                ax.set_xticks(x); ax.set_xticklabels(labels_d, rotation=45, ha="right")
                ax.bar_label(b1, padding=2, fontsize=8); ax.bar_label(b2, padding=2, fontsize=8)
                ax.legend(loc="upper right", fontsize=8)
                imgs["B_24h_dia"] = _save(fig, "B_24h_dia.png")
                # empilhado 100% normalizado
                nao_d = cheg_d - at24_d
                pct_at  = np.where(cheg_d > 0, 100.0 * at24_d / np.maximum(cheg_d, 1), 0.0)
                pct_nao = np.where(cheg_d > 0, 100.0 * nao_d / np.maximum(cheg_d, 1), 0.0)
                fig, ax = plt.subplots(figsize=(9.5, 4.6))
                ax.bar(x, pct_at, 0.6, color=COR_OK, label="Conversas atendidas")
                ax.bar(x, pct_nao, 0.6, bottom=pct_at, color=COR_ALERTA, label="Conversas não atendidas")
                for i, (pa, pn) in enumerate(zip(pct_at, pct_nao)):
                    if pa > 5:
                        ax.text(x[i], pa / 2, f"{pa:.0f}%", ha="center", va="center",
                                color="white", fontweight="bold", fontsize=10)
                ...
                ax.set_ylabel("% do total de chegadas"); ax.grid(axis="y", color="#E6EBF0")
                ax.legend(loc="upper right", fontsize=8)
                _ax(ax, "Conversas atendidas — distribuição proporcional por dia (%)")
                imgs["B_24h_dia_stacked"] = _save(fig, "B_24h_dia_stacked.png")
        except Exception as e: print("[g B_dia]", e)
        ...
        try:  # taxa de resposta diária — S.03
            col_assign_tx = achar_coluna(at, "assigned_at")
            if col_assign_tx:
                base_tx = at.dropna(subset=[col_assign_tx]).copy()
                base_tx["dia"] = base_tx[col_assign_tx].dt.floor("D")
                agg_tx = base_tx.groupby("dia").agg(
                    total=("respondido", "size"),
                    respondidas=("respondido", "sum"),
                ).reset_index()
                agg_tx["taxa_resp_pct"] = (100 * agg_tx["respondidas"] / agg_tx["total"]).round(1)
                media_tx = round(float(agg_tx["taxa_resp_pct"].mean()), 1)
                fig, ax = plt.subplots(figsize=(9.5, 4.6))
                xs = list(range(len(agg_tx)))
                ax.plot(xs, agg_tx["taxa_resp_pct"], color=COR_AZUL, marker="o",
                        markersize=7, linewidth=2.2,
                        markerfacecolor=COR_AZUL, markeredgecolor="white", markeredgewidth=1.5,
                        label="taxa de resposta (%)")
                ax.fill_between(xs, agg_tx["taxa_resp_pct"], alpha=0.10, color=COR_AZUL)
                ax.axhline(media_tx, color=COR_ALERTA, linewidth=1.5, linestyle="--",
                           label=f"média ({media_tx:.1f}%)")
                for xi, yi in zip(xs, agg_tx["taxa_resp_pct"]):
                    ax.annotate(f"{yi:.0f}%", (xi, yi),
                                textcoords="offset points", xytext=(0, 9),
                                ha="center", fontsize=8.5, color=COR_NAVY)
                ax.set_xticks(xs)
                ax.set_xticklabels(
                    [pd.Timestamp(d).strftime("%d/%m") for d in agg_tx["dia"]],
                ...
                _ax(ax, "Taxa de resposta diária (%)")
                imgs["D3_taxa_resp"] = _save(fig, "D3_taxa_resp_tempo.png")
        except Exception as e: print("[g D3_taxa]", e)
    ...
    if pv is not None and len(pv):
        ...
        # (a aba D1_por_vendedor preserva a linha para consistência dos dados)
    ...
    if pv is not None and len(pv):
        ...
        # período: com 0 recebidas o % é trivial (100% respondidas / 0% não
        # respondidas) e distorce o ranking
    ...
    if len(v):
        ...
        # % calculado sobre as sessões DO PRÓPRIO caminho (denominador por caminho)
        try:
            ...
            if comp_n is not None and len(comp_n):
                ...
                imgs["G2b_completude_novo"] = _barh_rotulado(cc["campo"], cc["%_preenchido"],
                    f"Preenchimento — Sou novo por aqui ({n_n} sessões neste caminho)",
                    "G2b_novo.png", sufixo="%")
        ...
        try:
            ...
            if comp_c is not None and len(comp_c):
                ...
                imgs["G2c_completude_cli"] = _barh_rotulado(cc["campo"], cc["%_preenchido"],
                    f"Preenchimento — Já sou cliente ({n_c} sessões neste caminho)",
                    "G2c_cli.png", sufixo="%")
...
ORDEM_PAINEL = [
    ...
    ("Chegada ao vendedor x conversas atendidas — semanal (absoluto)", "B_24h_semanal"),
    ...
    ("Tempo mediano de 1ª resposta (h)", "D1d_tempo"),
...
def exportar_resultados(blocos, imagens, caminho=ARQUIVO_SAIDA):
    ...
    with pd.ExcelWriter(caminho, engine="openpyxl") as w:
        ...
        ws["A1"] = "RELATÓRIO SEMANAL — TREBLE"
...
if __name__ == "__main__":
    ...
    _parser = _ap.ArgumentParser(
        description='Análise semanal Treble. Exemplo: python scripts/geral/analise_treble_semanal.py "07-06-2026 a 13-06-2026"'
    )
    _parser.add_argument(
        'periodo', nargs='?', default=None,
        metavar='DD-MM-AAAA a DD-MM-AAAA',
        help='Período da semana. Define ARQUIVO, SEMANA_INICIO e SEMANA_FIM automaticamente.'
    )
    ...
    if _args.periodo:
        ...
        if ARQUIVO is None:
            # pasta canônica da semana primeiro; fallback para o caminho literal
            _periodo_curto = f'{d1}-{m1}-{y1} a {d2}-{m2}-{y2}'
            _cand = f'analises/{_periodo_curto} (geral)/Dados_Treble_Semana.xlsx'
